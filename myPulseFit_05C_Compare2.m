function myPulseFit_05C_Compare()
    ...
    for i=1:n05
        ...
        %% 4.1 1RC 피팅
        % 파라미터: [R0, R1, C]
        init_1rc = [0.01, 0.02, 2.0];  % 임의
        lb_1rc   = [0,0,0];
        ub_1rc   = [0.5,1.0,100];

        problem_1rc = createOptimProblem('fmincon',...
            'objective',@(p) cost_1RC(p, t_data, dv_data, I_val),...
    ...
    figure('Name','1RC vs 2RC (No Reuse)','Color','w');
    set(gcf,'Units','pixels','Position',[100,100,1200,800]);
    sgtitle('0.5C Steps: Data vs 1RC vs 2RC (Independent)');
    ...
    for i=1:n05
        ...
        if subplot_idx>plots_per_fig
            fig_cnt=fig_cnt+1;
            figure('Name',['1RC vs 2RC part-',num2str(fig_cnt)],'Color','w');
            set(gcf,'Units','pixels','Position',[100,100,1200,800]);
            sgtitle('0.5C Steps: Data vs 1RC vs 2RC');
            subplot_idx=1;
        end
        subplot(4,4,subplot_idx); hold on; grid on;
        ...
        plot(t_data, dv_1rc, 'r--','LineWidth',1.2,'DisplayName','1RC');
        ...
        plot(t_data, dv_2rc, 'b-.','LineWidth',1.2,'DisplayName','2RC');

        title(['idx=',num2str(idx)],'FontSize',10);
...
function cost = cost_1RC(p, t, dv_data, I)
    dv_model = model_1RC(t,I,p);
    cost = sqrt(mean( (dv_data - dv_model).^2 ));
end
...
function cost = cost_2RC(p, t, dv_data, I)
