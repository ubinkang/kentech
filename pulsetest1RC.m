function myPulseFit_05C()
    ...
    for i = 1:Npulse
        ...
        % ✅ 파라미터 저장
        all_R0 = [all_R0; para_hat(1)];
        all_R1 = [all_R1; para_hat(2)];
        all_tau = [all_tau; para_hat(3)];
        all_SOC_begin = [all_SOC_begin; dataAll(i).SOC_begin];
        
        subplot_idx = subplot_idx + 1;
    end

    %% ✅ 3. 피팅된 파라미터와 SOC_begin 저장
    fitted_params.R0 = all_R0;
    fitted_params.R1 = all_R1;
    fitted_params.tau = all_tau;
    fitted_params.SOC_begin = all_SOC_begin;
    save('Fitted_1RC_Params.mat', 'fitted_params');

    %% ✅ 4. 피팅 결과 출력
    fprintf('총 %d개의 Pulse 데이터에 대한 1RC 모델 파라미터가 저장되었습니다.\n', length(all_R0));

    %% ✅ 5. R0, R1, tau vs SOC_begin 플로팅
    figure;
    subplot(3,1,1);
    scatter(all_SOC_begin, all_R0, 'b', 'filled');
    xlabel('SOC_{begin} [%]'); ylabel('R0 [Ω]');
    title('R0 vs SOC_{begin}'); grid on;

    subplot(3,1,2);
    scatter(all_SOC_begin, all_R1, 'r', 'filled');
    xlabel('SOC_{begin} [%]'); ylabel('R1 [Ω]');
    title('R1 vs SOC_{begin}'); grid on;

    subplot(3,1,3);
    scatter(all_SOC_begin, all_tau, 'g', 'filled');
    xlabel('SOC_{begin} [%]'); ylabel('\tau [s]');
    title('tau vs SOC_{begin}'); grid on;
end
